...
def test_product_fit_frames_physical_workcell_and_editor_anchors() -> None:
    ...
    assert "qMax(qMax(base_fit_distance * 1.30, product_radius * 2.80), 1.80)" in VIEWPORT
    assert "yaw_ = -0.86" in VIEWPORT
    assert "pitch_ = -0.60" in VIEWPORT
    assert "viewport->fit_product_view();" in MAINWINDOW


def test_product_view_defaults_hide_safety_and_use_subtle_floor_grid() -> None:
    assert "viewport->show_safety = false;" in MAINWINDOW
