...
const cameraClicked = state.sceneJson.sensors.find(item => item.canonical_scene_item_id === 'realsense_overhead')
  || state.sceneJson.sensors.find(item => item.id === 'realsense_overhead');
const tableClicked = state.sceneJson.assets.find(item => item.canonical_scene_item_id === 'support_surface_table')
...
const inspectionPreview = {...state.sceneJson.robot_preview, expected_robot_visual_links:['base_link'], expected_tool_visual_links:['robotiq_85_base_link']};
loadExpandedUrdfRobotPreview(inspectionPreview);
...
const armLink = inspectionPreview.expected_robot_visual_links[0];
const toolLink = inspectionPreview.expected_tool_visual_links[0];
...
const warnings=[]; const editPatches=[];
const context={console:{...console,warn(...args){warnings.push(args);}},warnings,editPatches,assert,window:{location:{search:''},dispatchEvent(){},parent:{postMessage(message){if(message?.type==='workcell_studio_edit_patch')editPatches.push(message);}}},document:{getElementById(){return element();},querySelectorAll(){return[];},createElement(){return element();}},URLSearchParams,CustomEvent:function(){},requestAnimationFrame(){},setTimeout(){},clearTimeout(){}};
vm.createContext(context);
vm.runInContext(source + `
const node = (name, children=[]) => { const value={name,children,visible:true,userData:{},parent:null}; for(const child of children)child.parent=value; return value; };
let callbacks=[];
loadRobotPreview=(_preview,options)=>{callbacks.push(options); return {diagnostics:{},links:new Map()};};
...
updateLabels=()=>{}; populateInspector=()=>{}; attachTransformGizmo=()=>{}; detachTransformGizmo=()=>{};
let highlighted=[]; refreshSelectionHighlight=record=>highlighted.push(record); removeSelectionHighlight=()=>{};
const ownerItems=[{id:'ur5'},{id:'robotiq_85_gripper'}];
state.sceneJson={scene:{id:'urdf_descendants'},objects:ownerItems}; state.objects=[];
state.dirtyTransforms=new Map(); state.undoStack=[]; state.redoStack=[]; state.editorEvents=[];
rebuildSelectionIdentityIndex();
const preview={robot_instance_id:'ur5',selection_robot_owner_id:'ur5',selection_tool_owner_id:'robotiq_85_gripper',expected_robot_visual_links:['base_link','wrist_link'],expected_tool_visual_links:['robotiq_85_base_link']};
loadExpandedUrdfRobotPreview(preview);
const mesh=node('base-mesh');
mesh.userData.item={id:'stale_generated_visual',diagnostic_only:true,selectable:false};
...
assert.strictEqual(state.pickRecords.length,3);
...
const toolRecord=state.pickRecords.find(record=>record.item.link_name==='robotiq_85_base_link');
assert.strictEqual(baseRecord.readOnlyPick,true);
...
assert.notStrictEqual(itemFromRaycastHit({object:mesh})?.item?.id,'stale_generated_visual','stale descendant metadata must not terminate registered inspection traversal');
...
for (const record of [baseRecord,childRecord,toolRecord]) {
  ...
  assert.strictEqual(record.readOnlyPick,true);
}
const nameSpoof={item:{id:'ur5_wrist_link_visual'},object3d:{visible:true},readOnlyPick:true};
state.pickRecords.push(nameSpoof);
assert.strictEqual(isExpandedUrdfInspectionPick(nameSpoof),false,'names and link substrings do not grant inspection eligibility');
...
// The six completion diagnostics distinguish records, bound descendants, and arm/tool ownership.
assert.strictEqual(state.robotUrdfPreviewDiagnostics.robot_pick_record_count,3);
assert.strictEqual(state.robotUrdfPreviewDiagnostics.expanded_urdf_pick_record_count,3);
assert.strictEqual(state.robotUrdfPreviewDiagnostics.expanded_urdf_unique_link_record_count,3);
assert.strictEqual(state.robotUrdfPreviewDiagnostics.expanded_urdf_bound_node_count,5);
assert.strictEqual(state.robotUrdfPreviewDiagnostics.expanded_urdf_robot_pick_record_count,2);
assert.strictEqual(state.robotUrdfPreviewDiagnostics.expanded_urdf_tool_pick_record_count,1);
assert.deepStrictEqual(Array.from(state.robotUrdfPreviewDiagnostics.expanded_urdf_robot_pick_record_links),['base_link','wrist_link']);
assert.deepStrictEqual(Array.from(state.robotUrdfPreviewDiagnostics.expanded_urdf_tool_pick_record_links),['robotiq_85_base_link']);
assert.strictEqual(state.robotUrdfPreviewDiagnostics.selection_robot_owner_id,'ur5');
assert.strictEqual(state.robotUrdfPreviewDiagnostics.selection_tool_owner_id,'robotiq_85_gripper');
...
assert.strictEqual(buildEditPatch().edits.length,0);
assert.strictEqual(editPatches.length,0);
assert.strictEqual(state.pickRecords.some(record=>state.objects.includes(record)),false);

// A rejected hit emits its complete diagnostic only once for the same signature.
const rejected=node('rejected-stale-mesh'); rejected.userData.diagnostic_only=true;
state.three={pointer:{},camera:{},raycaster:{setFromCamera(){},intersectObjects(){return [{object:rejected,distance:1}];}}};
assert.strictEqual(pickObject({clientX:5,clientY:5}),'');
assert.strictEqual(pickObject({clientX:5,clientY:5}),'');
assert.strictEqual(warnings.length,1,'one warning is emitted per diagnostic signature');
assert.strictEqual(warnings[0].length,1,'canvas pick warning uses one console argument');
const warning=warnings[0][0];
assert.strictEqual(typeof warning,'string');
assert.ok(warning.startsWith('Product View canvas pick rejected: '));
assert.ok(!warning.includes('[object Object]'));
...
const staleLink=node('stale-link',[node('stale-descendant')]);
state.sceneJson={scene:{id:'replacement_scene'},objects:ownerItems};
loadExpandedUrdfRobotPreview(preview);
const before=state.pickRecords.length;
staleCallback.onRobotLoaded({root:node('stale-root',[staleLink]),links:new Map([['base_link',staleLink]]),diagnostics:{}});
assert.strictEqual(state.pickRecords.length,before);
assert.strictEqual(state.pickIdentityByObject.get(staleLink),undefined);
assert.strictEqual(state.pickIdentityByObject.get(staleLink.children[0]),undefined);
`, context);
"""
    subprocess.run(
...
state.objects=[camera,table,bin];state.sceneJson={scene:{id:'ur5_2f_test'},objects:state.objects.map(record=>record.item),ui_selection_owners:[ur5Owner,toolOwner,camera.item,table.item,bin.item]};rebuildSelectionIdentityIndex();state.editorEvents=[];state.editorMode='select';
assert.ok(!state.objects.some(record=>record.item.id==='ur5'||record.item.id==='robotiq_85_gripper'));
assert.ok(!state.pickRecords.some(record=>record.item.id==='ur5'||record.item.id==='robotiq_85_gripper'));
state.sceneJson.robot_preview={selection_robot_owner_id:'ur5',selection_tool_owner_id:'robotiq_85_gripper',expected_robot_visual_links:['forearm_link','wrist_3_link'],expected_tool_visual_links:['robotiq_85_base_link']};
const generatedParent=new THREE.Group();generatedParent.name='generated_robot_parent';generatedParent.userData={diagnostic_only:true,non_selectable:true,selectable:false,source_layer:'generated_urdf',item:{id:'stale_generated_parent',source_layer:'generated_urdf',diagnostic_only:true,selectable:false,render_policy:'diagnostic_only'}};
...
const ur5Record=registerPickRecord({id:'urdf_visual_3_forearm_link::generated_visual',link_name:'forearm_link',diagnostic_only:true,selectable:false,render_policy:'diagnostic_only'},ur5Link,generatedParent,{pickRecordSource:'payload_item',authoritativePhysicalPick:true});
const toolRecord=registerPickRecord({id:'urdf_visual_robotiq_descendant',link_name:'robotiq_85_base_link',diagnostic_only:true,selectable:false,render_policy:'diagnostic_only'},toolLink,generatedParent,{pickRecordSource:'payload_item',authoritativePhysicalPick:true});
...
const generatedCameraItem={id:'generated_urdf::camera_link::visual_17::17',camera_id:'realsense_overhead',type:'camera',role:'sensor',locked:true,editable:false,selectable:true,source_layer:'generated_urdf'};
const generatedCameraParent=new THREE.Group();generatedCameraParent.name='generated_urdf_camera_link';generatedCameraParent.userData={diagnostic_only:true,non_selectable:true,selectable:false,source_layer:'generated_urdf',item:{id:'generated_camera_parent',source_layer:'generated_urdf',diagnostic_only:true,selectable:false}};
camera.object3d.remove(camera.mesh);generatedCameraParent.add(camera.mesh);camera.object3d.add(generatedCameraParent);
...
assert.strictEqual(state.lastCanvasSelectedItemId,'realsense_overhead','generated camera geometry emits only the authored camera owner');
...
assert.notStrictEqual(state.selected,'generated_urdf::camera_link::visual_17::17');
...
state.editorMode='select'; state.selected='target_bin_default'; state.three.raycaster={setFromCamera(){},intersectObjects(){return[];}}; pickObject({clientX:10,clientY:10}); assert.strictEqual(state.selected,''); assert.strictEqual(state.lastCanvasPickReason,'empty_select_click'); assert.strictEqual(warnings.length,0,'empty-space clicks do not warn');
...
assert.strictEqual(warnings[0].length,1);
assert.ok(warnings[0][0].startsWith('Product View canvas pick rejected: '));
assert.ok(!warnings[0][0].includes('[object Object]'));
assert.strictEqual(JSON.parse(warnings[0][0].slice('Product View canvas pick rejected: '.length)).nearest_known_urdf_link_ancestor,'known_link');
...
state.objects=[target,zone,robot,commissioning]; state.sceneJson={scene:{id:'ur5_2f_test'}}; state.debugOverlaysVisible=true;
...
assert.strictEqual(state.placement.yaw,Math.PI/12,'form controls reserve their keyboard input');
...
onCanvasPointerDown({button:0,shiftKey:true,clientX:30,clientY:30});assert.strictEqual(state.editorEvents.length,0);assert.strictEqual(state.placement.armed,true);assert.strictEqual(state.placement.previewRoot,ghost);assert.strictEqual(state.placement.yaw,Math.PI/12);
placementPointFromViewport=()=>({x:7,y:8,z:9});onCanvasPointerMove({clientX:40,clientY:40});onCanvasPointerDown({button:0,shiftKey:true,clientX:40,clientY:40,preventDefault(){},stopPropagation(){}});
let requests=state.editorEvents.filter(e=>e.type==='placement_requested');assert.strictEqual(requests.length,1);assert.deepStrictEqual([requests[0].x,requests[0].y,requests[0].z,requests[0].yaw,requests[0].repeat],[7,8,9,Math.PI/12,true]);assert.strictEqual(state.placement.previewRoot,ghost);assert.strictEqual(state.placement.armed,true);assert.strictEqual(state.placement.yaw,Math.PI/12);
placementPointFromViewport=()=>({x:10,y:11,z:12});onCanvasPointerDown({button:0,shiftKey:true,clientX:45,clientY:45,preventDefault(){},stopPropagation(){}});requests=state.editorEvents.filter(e=>e.type==='placement_requested');assert.strictEqual(requests.length,2);assert.strictEqual(requests[1].yaw,Math.PI/12);assert.strictEqual(state.placement.previewRoot,ghost);
onEditorKeyDown({key:'Escape',code:'Escape',target:null,preventDefault(){}});assert.strictEqual(state.placement.armed,false);assert.strictEqual(state.placement.previewRoot,null);assert.strictEqual(state.placement.yaw,0);assert.strictEqual(requests.length,2);
armPlacement({asset});key('KeyQ');placementPointFromViewport=()=>({x:13,y:14,z:15});onCanvasPointerDown({button:0,shiftKey:false,clientX:50,clientY:50,preventDefault(){},stopPropagation(){}});requests=state.editorEvents.filter(e=>e.type==='placement_requested');assert.strictEqual(requests.length,3);assert.strictEqual(requests[2].yaw,-Math.PI/12);assert.strictEqual(requests[2].repeat,false);assert.strictEqual(state.placement.previewRoot,null);assert.strictEqual(state.placement.armed,false);assert.strictEqual(state.placement.yaw,0);
`,sandbox);
'''
...
setPlacementPoint({x:.823,y:-.117,z:.745});cancelPlacement();assert.deepStrictEqual(getPlacementState(),{armed:false,persistent:false,supportValid:false,collision:false,collidingOwnerIds:[],valid:null,proposedPoint:null});assert.strictEqual(el.placementStatus.hidden,true);assert.strictEqual(el.placementStatus.textContent,'');
...
def test_placement_collision_broad_phase_feedback_and_commit_contract():
    js_path = VIEWER / "viewer.js"
    harness = r'''
const fs=require('fs'),vm=require('vm'),assert=require('assert');let source=fs.readFileSync(process.argv[1],'utf8').replace(/boot\(\);\s*$/,'');
...
setPlacementPoint({x:1.1,y:0,z:.5});assert.strictEqual(state.placement.collision,true);assert.strictEqual(state.placement.valid,false);assert.deepStrictEqual(state.placement.collidingOwnerIds,['object_01']);
const helpers=['fallback_edges','selection_outline','TransformControls','camera_frustum'].map(id=>{const root=new THREE.Group();root.name=id;root.userData.exclude_from_physical_bounds=true;root.add(new THREE.Mesh(new THREE.BoxGeometry(4,4,4),new THREE.MeshBasicMaterial()));state.three.scene.add(root);return root});evaluatePlacementCollision();assert.deepStrictEqual(state.placement.collidingOwnerIds,['object_01'],'helpers never become owners');
object01.object3d.position.x=5;const table=owner('table',0,[10,10,1],{role:'support_surface',category:'table'});const bin=owner('target_bin_default',0,[.4,.4,1]);setPlacementPoint({x:0,y:0,z:.5});assert(!state.placement.collidingOwnerIds.includes('table'),'active support owner ignored');assert.deepStrictEqual(state.placement.collidingOwnerIds,['target_bin_default'],'canonical owner, not descendant mesh, is reported');
const imported=new THREE.Mesh(new THREE.BoxGeometry(1000,1000,1000),new THREE.MeshBasicMaterial());const transform=meshLocalTransformOf({mesh_scale:[.001,.001,.001]});imported.scale.copy(transform.scale);imported.updateWorldMatrix(true,true);const scaled=new THREE.Box3().setFromObject(imported);assert(Math.abs(scaled.max.x-scaled.min.x-1)<1e-9,'mesh scale affects bounds exactly once');
bin.object3d.position.set(.75,.75,.5);ghost.scale.set(2,.4,1);ghost.rotation.z=0;setPlacementPoint({x:0,y:0,z:.5});assert.strictEqual(state.placement.collision,false);state.placement.yaw=Math.PI/4;ghost.rotation.z=state.placement.yaw;evaluatePlacementCollision();assert.strictEqual(state.placement.collision,true,'yaw recomputes current world AABB without pointer movement');
ghost.scale.set(1,1,1);ghost.rotation.z=0;state.placement.yaw=0;bin.object3d.position.set(1.1,0,.5);el.snapToggle.checked=true;el.translationSnap.value='1';setPlacementPoint({x:.6,y:0,z:.5});assert.strictEqual(state.placement.proposedPoint.x,1);assert.strictEqual(state.placement.collision,true,'collision uses snapped pose');
placementPointFromViewport=()=>({x:.6,y:0,z:.5});let before=state.editorEvents.length;onCanvasPointerDown({button:0,shiftKey:false,clientX:1,clientY:1});onCanvasPointerDown({button:0,shiftKey:true,clientX:1,clientY:1});assert.strictEqual(state.editorEvents.length,before,'normal and repeat clicks are blocked');assert.strictEqual(state.placement.armed,true);
bin.object3d.position.x=4;placementPointFromViewport=()=>({x:0,y:0,z:.5});onCanvasPointerDown({button:0,shiftKey:true,clientX:1,clientY:1,preventDefault(){},stopPropagation(){}});let requests=state.editorEvents.filter(event=>event.type==='placement_requested');assert.strictEqual(requests.length,1,'moving clear commits exactly once');assert.strictEqual(state.placement.armed,true);
