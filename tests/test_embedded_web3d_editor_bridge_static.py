...
def test_selection_diagnostics_are_exposed_to_qt_bridge():
    ...
    assert "selectionDiagnostics: () => currentSelectionDiagnostics()" in VIEWER
    for field in ["renderIdentity", "sourceLayer", "activeVisualSource", "diagnosticOnly", "helperOrOverlay", "objectPresent", "canonicalSelectedId", "editAuthoritySource", "gizmoAttachedTargetId", "gizmoVisible", "gizmoEnabled", "gizmoAttachmentReason", "physicalEditBinding", "canonicalOwnerId", "ownerRecordSource", "physicalVisualId", "pivotWorldCentre", "attachedObjectWorldPosition", "distance", "attachmentReason"]:
        assert field in VIEWER
...
hits=[{object:bin.object3d,distance:1}];assert.strictEqual(pickObject({clientX:5,clientY:5}),'target_bin_default');assert.strictEqual(gizmo.object,bin.object3d);assert.strictEqual(state.gizmoPivot,null);
