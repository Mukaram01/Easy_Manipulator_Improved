...
const cameraMeshCentre=()=>new THREE.Box3().setFromObject(cameraVisual.meshObject).getCenter(new THREE.Vector3());assert(cameraPivot.position.distanceTo(cameraMeshCentre())<1e-12);
const helper=new THREE.Mesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshBasicMaterial());helper.name='camera_fov_frustum_helper';helper.position.set(50,50,50);camera.object3d.add(helper);assert(authoritativePhysicalMeshCentre(camera).distanceTo(cameraMeshCentre())<1e-12,'camera helper must not affect physical centre');
