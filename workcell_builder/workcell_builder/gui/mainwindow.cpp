...
bool MainWindow::preview_command_is_safe(const QString & command, QStringList * blockers) const
{
  ...
  if (!safe && blockers) blockers->append("command rejected by fake-hardware safety guard");
  return safe;
}
...
void MainWindow::run_preview_build(){ QStringList blockers; if(!selected_scene_preview_ready(&blockers)){ append_studio_log("Preview build blocked: " + blockers.join(" | ")); QMessageBox::warning(this,"Plan & Simulate",blockers.join("\n")); return; } if(detect_workspace_root().isEmpty()){ append_studio_log("Preview build blocked: install/setup.bash missing under workspace root"); QMessageBox::warning(this,"Preview Launch","Workspace root not detected. Copy commands and run manually."); return;} active_preview_command_=selected_scene_build_command(); append_studio_log("Preview build started: " + active_preview_command_); if(preview_log_) preview_log_->appendPlainText("$ "+active_preview_command_); set_preview_state("BUILD_RUNNING"); write_preview_launch_transcript(true, active_preview_command_, "build_started"); preview_process_->start("/bin/bash", {"-lc", active_preview_command_}); }
void MainWindow::run_fake_hardware_preview(){ QStringList blockers; if(!selected_scene_preview_ready(&blockers)){ append_studio_log("Preview launch blocked: " + blockers.join(" | ")); QMessageBox::warning(this,"Plan & Simulate",blockers.join("\n")); return; } QString command = "cd "+detect_workspace_root()+" && source install/setup.bash && "+selected_scene_launch_command(); if(!preview_command_is_safe(command,&blockers)){ append_studio_log("Preview dry-run blocked: " + blockers.join(" | ")); QMessageBox::warning(this,"Plan & Simulate",blockers.join("\n")); return; } append_studio_log("Preview dry-run passed"); auto rc = QMessageBox::question(this,"Confirm Fake-Hardware Preview", "Command:\n"+command+"\n\nFake hardware only. No real hardware. No runtime execution. No robot motion commanded."); if(rc!=QMessageBox::Yes) return; append_studio_log("Preview launch started: " + command); active_preview_command_=command; if(preview_log_) preview_log_->appendPlainText("$ "+command); set_preview_state("PREVIEW_RUNNING"); write_preview_launch_transcript(true, command, "preview_started"); preview_process_->start("/bin/bash", {"-lc", command}); refresh_new_cell_checklist(); }
