...
connect(run_demo, &QPushButton::clicked, this, [this](){ append_studio_log("Demo readiness completed"); });
  ...
  connect(preview_process_, &QProcess::started, this, [this]() {
    const qint64 pid = preview_process_ ? preview_process_->processId() : 0;
    const QString state = preview_process_ && preview_process_->state() != QProcess::NotRunning ? "running" : "not_running";
    append_studio_log(QString("Preview process started (scene=%1 pid=%2 state=%3)")
      .arg(active_preview_scene_name_.isEmpty() ? "<unknown>" : active_preview_scene_name_)
      .arg(pid)
      .arg(state));
  });
...
void MainWindow::run_preview_build(){ QStringList blockers; if(!selected_scene_preview_ready(&blockers)){ QMessageBox::warning(this,"Plan & Simulate",blockers.join("\n")); return; } if(detect_workspace_root().isEmpty()){ QMessageBox::warning(this,"Preview Launch","Workspace root not detected. Copy commands and run manually."); return;} if (!preview_process_ || preview_process_->state() != QProcess::NotRunning) { append_studio_log("WARN Preview build request ignored: another preview process is already running."); return; } active_preview_scene_name_ = (selected_scene_index_ >= 0 && selected_scene_index_ < static_cast<int>(scene_browser_result_.scenes.size())) ? QString::fromStdString(scene_browser_result_.scenes[static_cast<size_t>(selected_scene_index_)].scene_name) : QString(); active_preview_command_=selected_scene_build_command(); if(preview_log_) preview_log_->appendPlainText("$ "+active_preview_command_); set_preview_state("BUILD_RUNNING"); write_preview_launch_transcript(true, active_preview_command_, "build_started"); preview_process_->start("/bin/bash", {"-lc", active_preview_command_}); }
void MainWindow::run_fake_hardware_preview(){ QStringList blockers; if(!selected_scene_preview_ready(&blockers)){ QMessageBox::warning(this,"Plan & Simulate",blockers.join("\n")); return; } const QString requested_scene_name = (selected_scene_index_ >= 0 && selected_scene_index_ < static_cast<int>(scene_browser_result_.scenes.size())) ? QString::fromStdString(scene_browser_result_.scenes[static_cast<size_t>(selected_scene_index_)].scene_name) : QString(); if (preview_process_ && preview_process_->state() != QProcess::NotRunning) { if (preview_state_ == "PREVIEW_RUNNING" && requested_scene_name == active_preview_scene_name_) { append_studio_log(QString("WARN Duplicate preview start ignored for scene '%1' (existing preview still running).").arg(requested_scene_name)); return; } append_studio_log("WARN Preview start ignored: another preview process is already running."); return; } QString command = "cd "+detect_workspace_root()+" && source install/setup.bash && "+selected_scene_launch_command(); if(!preview_command_is_safe(command,&blockers)){ QMessageBox::warning(this,"Plan & Simulate",blockers.join("\n")); return; } auto rc = QMessageBox::question(this,"Confirm Fake-Hardware Preview", "Command:\n"+command+"\n\nFake hardware only. No real hardware. No runtime execution. No robot motion commanded."); if(rc!=QMessageBox::Yes) return; active_preview_scene_name_ = requested_scene_name; active_preview_command_=command; if(preview_log_) preview_log_->appendPlainText("$ "+command); set_preview_state("PREVIEW_RUNNING"); write_preview_launch_transcript(true, command, "preview_started"); preview_process_->start("/bin/bash", {"-lc", command}); refresh_new_cell_checklist(); }
...
void MainWindow::handle_preview_stdout(){ if (!preview_process_) return; const QString out = QString::fromUtf8(preview_process_->readAllStandardOutput()); if(out.isEmpty()) return; if(preview_log_) preview_log_->appendPlainText(out); append_studio_log("[preview stdout] " + out.trimmed()); }
void MainWindow::handle_preview_stderr(){ if (!preview_process_) return; const QString err = QString::fromUtf8(preview_process_->readAllStandardError()); if(err.isEmpty()) return; if(preview_log_) preview_log_->appendPlainText(err); append_studio_log("[preview stderr] " + err.trimmed()); }
void MainWindow::handle_preview_finished(int exit_code, QProcess::ExitStatus exit_status){ if(preview_state_=="BUILD_RUNNING") set_preview_state(exit_code==0?"BUILD_PASSED":"BUILD_FAILED"); else if(preview_state_=="PREVIEW_STOPPING") set_preview_state("PREVIEW_STOPPED"); else set_preview_state(exit_code==0?"PREVIEW_EXITED":"PREVIEW_FAILED"); const QString status_text = (exit_status == QProcess::NormalExit) ? "NormalExit" : "CrashExit"; append_studio_log(QString("Preview process finished (scene=%1 exit_code=%2 exit_status=%3)").arg(active_preview_scene_name_.isEmpty() ? "<unknown>" : active_preview_scene_name_).arg(exit_code).arg(status_text)); if (exit_status != QProcess::NormalExit) append_studio_log("WARN Preview process crashed or terminated unexpectedly."); write_preview_launch_transcript(true, active_preview_command_, "process_finished", exit_code); active_preview_scene_name_.clear(); refresh_new_cell_checklist(); }
