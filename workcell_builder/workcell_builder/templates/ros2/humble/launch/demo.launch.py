...
def _launch_setup(context):
    ...
    if octomap_enabled:
        ...
        octomap_launch_message = LogWarning(
            msg=(
                "enable_octomap:=true requested, but no 3D sensor plugin configuration "
                "is generated by default. Add sensor_3d.yaml-style occupancy map sensor "
                "parameters before expecting octomap updates."
            )
        )
    else:
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            default_value="false",
            description=(
                "Enable MoveIt octomap integration. Defaults to false so generated "
                "scenes do not require 3D sensor plugin configuration."
            ),
        ),
        DeclareLaunchArgument(
            "use_fake_hardware",
