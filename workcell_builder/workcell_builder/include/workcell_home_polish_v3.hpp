...
// Stable Home composition.  This pass deliberately stays inside the proven v3
// startup path: v2 owns the canonical table/model and v3 only presents that
// data cleanly.  No delegate replacement, no cell-widget deletion, and no
// hidden 3D scene loading is performed from Home.
...
inline QString canonicalDirectory(const QString & path)
{
  const QFileInfo info(path);
  if (!info.exists() || !info.isDir()) return QString();
...
{
  QStringList roots;
  const auto add_root = [&roots](const QString & path) {
    const QString root = canonicalDirectory(path);
  ...
  if (window) {
    if (auto * summary = window->findChild<QLabel *>(QStringLiteral("dashboardSummaryLabel"))) {
      ...
      if (match.hasMatch()) add_root(match.captured(1).trimmed());
    }
  ...
  QSettings startup(QStringLiteral("easy_manipulation_deployment"), QStringLiteral("workcell_builder"));
  const QString workspace = startup.value(QStringLiteral("startup/last_workspace")).toString().trimmed();
  ...
  const QString home_workspace = QDir::homePath() + QStringLiteral("/workcell_ws");
  add_root(QDir(home_workspace).filePath(QStringLiteral("src/easy_manipulation_deployment/scenes")));
  add_root(QDir(home_workspace).filePath(QStringLiteral("src/scenes")));
...
inline QString cachedPreviewPath(QMainWindow * window, const QString & scene)
{
  ...
  const QStringList exact_candidates = {
  ...
  for (const QString & root : sceneRootCandidates(window)) {
    ...
    for (const QString & relative : exact_candidates) {
    ...
    for (const QString & subdir : {QStringLiteral("smoke"), QStringLiteral("acceptance"),
      ...
      for (const QString & pattern : patterns) {
        const QStringList matches = image_dir.entryList({pattern}, QDir::Files, QDir::Time);
...
inline QPixmap fitPreview(const QPixmap & source, const QSize & size)
{
...
inline QPixmap liveCanonicalPreview(QMainWindow * window, const QString & scene, const QSize & size)
{
  ...
  if (!source || !source->isVisible() || source->width() < 48 || source->height() < 32) {
    source = source_preview->findChild<QWidget *>(QStringLiteral("scene3dViewportWidget"));
  }
  if (!source || !source->isVisible() || source->width() < 48 || source->height() < 32) return QPixmap();
  const QPixmap shot = source->grab();
  if (!pixmapHasUsefulVariation(shot)) return QPixmap();
  return fitPreview(shot, size);
}
...
inline void refreshInspectorPreview(QMainWindow * window, const QString & scene)
{
  ...
  const QSize target(qMax(300, preview->width() - 8), qMax(204, preview->height() - 8));

  // Prefer an already-generated scene image.  This is deterministic on Home and
  // works with the workspace symlink layout (~/workcell_ws/src/.../scenes).
  const QString cached_path = cachedPreviewPath(window, scene);
  const QPixmap cached(cached_path);
  if (!cached.isNull()) {
    preview->setText(QString());
    preview->setPixmap(fitPreview(cached, target));
    preview->setToolTip(cached_path);
    return;
  }

  // Only sample Product View when it is already visible and usable. Home never
  // force-loads a hidden WebEngine/OpenGL surface.
  ...
  preview->setText(scene.isEmpty()
    ...
    : QStringLiteral("NO PREVIEW IMAGE\nOpen Product View once to render this workcell"));
  preview->setToolTip(QStringLiteral("No smoke, acceptance, preview, or currently visible Product View image is available."));
}
...
inline void polishTable(QMainWindow * window)
{
  ...
  // v2 owns the table and its single status-pill cell widget.  Do not add a
  // second QWidget presentation over the same canonical QTableWidgetItem data.
  // That overlap was the source of the duplicated text in workstation screenshots.
  home_polish_v2::polishTable(window);
  ...
  table->setColumnWidth(kToolColumn, 166);
  table->setColumnWidth(kUpdatedColumn, 120);
  ...
  for (int row = 0; row < table->rowCount(); ++row) table->setRowHeight(row, 46);
}
...
inline int firstVisibleRow(QTableWidget * table)
{
  if (!table) return -1;
  for (int row = 0; row < table->rowCount(); ++row) {
    if (!table->isRowHidden(row)) return row;
  }
  return -1;
}
...
inline void ensureUsefulSelection(QMainWindow * window)
{
  ...
  if (table->currentRow() >= 0 && table->currentRow() < table->rowCount() &&
      !table->isRowHidden(table->currentRow())) return;

  QSettings settings;
  int row = rowForScene(table, settings.value(QStringLiteral("studio_home/last_selected_scene")).toString());
  if (row < 0) {
    for (const QString & pinned : pinnedScenes()) {
...
inline void updateInspector(QMainWindow * window)
{
  ...
  if (!has_scene) {
    ...
    refreshInspectorPreview(window, QString());
    return;
  }
...
inline void buildInspector(QMainWindow * window)
{
  ...
  details_panel->setMaximumWidth(410);
  ...
  meta->setContentsMargins(2, 3, 2, 3);
  ...
  addMetaRow(meta, 3, meta_host, QStringLiteral("◷"), QStringLiteral("Last Updated"), QStringLiteral("homeV3MetaUpdated"));
...
inline void polishShell(QMainWindow * window)
{
  ...
  if (auto * nav = window->findChild<QListWidget *>(QStringLiteral("studioHomeSidebarNav"))) {
    ...
    if (nav->count() > 2) nav->item(2)->setHidden(true);
    if (nav->count() > 3) nav->item(3)->setHidden(true);
...
QLabel#homeV3InspectorPreview { background:#121D28; color:#C7D3DF; border:1px solid #243747; border-radius:8px; font-size:11px; font-weight:700; padding:4px; }
...
QLabel#homeV3MetaRobot, QLabel#homeV3MetaTool, QLabel#homeV3MetaTask, QLabel#homeV3MetaUpdated { color:#17324C; font-size:11px; font-weight:800; }
...
QToolButton#homeV3InspectorPin { background:transparent; color:#173B5D; border:0; font-size:19px; padding:3px 6px; }
QToolButton#homeV3InspectorPin:hover { background:#EDF3F8; border-radius:5px; }
QTableWidget#studioHomeSceneTable { border:1px solid #DCE5EF; border-radius:8px; background:#FFFFFF; outline:0; }
QTableWidget#studioHomeSceneTable::item { border-bottom:1px solid #EEF2F6; padding:7px 10px; color:#253A50; }
QTableWidget#studioHomeSceneTable::item:selected { background:#EAF2FB; color:#163A5C; }
QTableWidget#studioHomeSceneTable QHeaderView::section { background:#F8FAFC; border:0; border-bottom:1px solid #DCE5EF; padding:8px 10px; color:#465A70; font-weight:800; }
...
protected:
  bool eventFilter(QObject * watched, QEvent * event) override
  {
    ...
    if (event->type() == QEvent::Show) {
      // v2 builds the canonical model first. v3 only presents it afterwards.
      QTimer::singleShot(760, window, [window]() { applyPolish(window); });
      QTimer::singleShot(1350, window, [window]() { applyPolish(window); });
    } else if (event->type() == QEvent::Resize) {
      QTimer::singleShot(0, window, [window]() { polishShell(window); polishTable(window); });
    }
    return QObject::eventFilter(watched, event);
  }
};
