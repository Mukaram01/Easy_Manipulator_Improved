...
void NextPointPublisher::configure(
  ...
  command_out_vel_.resize(dof);
  command_out_eff_.resize(dof);
...
void NextPointPublisher::start(double scale) {
  ...
  //   std::bind(&NextPointPublisher::_deadline_cb, this,
  //   std::placeholders::_1);
...
void NextPointPublisher::_send_command() {
  ...
  for (size_t i = 0; i < command_out_.size(); i++) {
    ...
    if (publish_joint_velocity_) {
      command_out_vel_[i] = *point_->getJointVelocities(joints[i]);
    }
    if (publish_joint_effort_) {
      command_out_eff_[i] = *point_->getJointEfforts(joints[i]);
    }
  }
  switch (command_out_type_) {
  case Command::ARRAY: {
