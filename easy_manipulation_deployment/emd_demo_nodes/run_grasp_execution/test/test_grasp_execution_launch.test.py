...
class TestGraspExecutionLaunch(unittest.TestCase):
    ...
    def _topic_available(self, topic_name):
        return any(
            name == topic_name for name, _types in self.node.get_topic_names_and_types()
        )

    def test_required_nodes_and_topics(self):
        ...
        self.assertTrue(self._wait_for(lambda: self._topic_available("/joint_states")))
