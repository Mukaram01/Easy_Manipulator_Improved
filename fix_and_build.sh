...
# Generate a minimal CMake package configuration for tesseract_motion_planners_core
...
ensure_tesseract_motion_planners_core_config() {
  ...
  local targets_file="$config_dir/tesseract_motion_planners_coreTargets.cmake"
  ...
  if [[ ! -f "$config_file" ]]; then
    ...
    if [[ ! -f "$targets_file" ]]; then
      needs_regeneration=1
    elif ! grep -Eq "add_library\\(tesseract_motion_planners_core" "$config_file" "$targets_file"; then
      needs_regeneration=1
    fi

    if (( needs_regeneration )); then
      echo "Replacing incomplete tesseract_motion_planners_core package configuration under $config_dir"
    fi
  fi
  ...
  local candidate
  for candidate in \
    "$package_prefix/lib/libtesseract_motion_planners_core.so" \
    "$package_prefix/lib/libtesseract_motion_planners_core.dylib" \
    "$package_prefix/lib/tesseract_motion_planners_core.lib" \
    "$package_prefix/bin/tesseract_motion_planners_core.dll"; do
    if [[ -f "$candidate" ]]; then
      core_lib="$candidate"
      break
    fi
  ...
  if [[ -z "$core_lib" ]]; then
    echo "tesseract_motion_planners_core library not found under $package_prefix; cannot synthesize CMake config" >&2
    return
  fi
...
if(NOT TARGET tesseract_motion_planners_core)
  add_library(tesseract_motion_planners_core SHARED IMPORTED)
  set_target_properties(tesseract_motion_planners_core PROPERTIES
    IMPORTED_LOCATION "${core_lib}"
    INTERFACE_INCLUDE_DIRECTORIES "${include_dir}")
endif()
if(NOT TARGET tesseract::tesseract_motion_planners_core)
  add_library(tesseract::tesseract_motion_planners_core ALIAS tesseract_motion_planners_core)
endif()
EOF

  cat >"$config_file" <<EOF
# Generated by fix_and_build.sh to provide a minimal CMake package for
# tesseract_motion_planners_core when the upstream build does not install one.
include(CMakeFindDependencyMacro)
find_dependency(Eigen3)
find_dependency(console_bridge)
find_dependency(tesseract_common)
...
include("\${CMAKE_CURRENT_LIST_DIR}/tesseract_motion_planners_coreTargets.cmake")
if(NOT TARGET tesseract::tesseract_motion_planners_core)
  add_library(tesseract::tesseract_motion_planners_core ALIAS tesseract_motion_planners_core)
endif()
set(tesseract_motion_planners_core_FOUND TRUE)
EOF
...
PY
  ...
  local version_file="$config_dir/tesseract_motion_planners_coreConfigVersion.cmake"
  local version_script
  version_script=$(mktemp)
  cat >"$version_script" <<'EOF'
include(CMakePackageConfigHelpers)
...
EOF
  ...
  echo "Synthesized tesseract_motion_planners_core package configuration at $config_dir"
}
...
# Build motion planners once up front so their exported CMake package is
# discoverable before configuring downstream composer packages.
colcon build "${COLCON_BASE_ARGS[@]}" --packages-select tesseract_motion_planners --cmake-args "${CMAKE_ARGS[@]}"
source_install
ensure_tesseract_motion_planners_core_config
