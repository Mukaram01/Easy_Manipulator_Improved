...
def load_canonical_place_target(package_share, destination_zone=None):
    ...
    zone = matches[0]
    if zone.get('type') != 'place_zone' or zone.get('role', 'place_zone') != 'place_zone':
...
def place_motion_targets(tool_at_grasp, target, destination, retreat):
    ...
    return (translated_pose(tool_at_grasp, delta[0], delta[1], delta[2] + retreat),
...
def eligible_grasp_target(target, destination):
    """Existing 2F/destination bounds gate; MoveIt decides actual feasibility."""
...
def main():
    ...
    try:
        ...
        summary['place_target'] = destination
        ...
        def preplan(target, index, record):
            ...
            try:
                ...
                transfer_pose, place_pose = place_motion_targets(tool_at_grasp, target, destination, retreat)
