...
def generate_emd_planner_files(state:dict[str,Any], scenario_name:str, gripper_type:str, planner_values:dict[str,Any], planner_dir:Path|None=None)->dict[str,Any]:
...
def validate_generated_emd_planner_config(payload:dict[str,Any])->dict[str,Any]:
    ...
    if epd.get("epd_enabled") and not epd: errors.append("missing easy_perception_deployment when epd_enabled true")
    if not pc.get("point_cloud_topic"): errors.append("missing point_cloud_topic")
    ...
    if cam.get("fx",0)==0 or cam.get("fy",0)==0: warnings.append("camera intrinsics are default")
