...
def main() -> int:
    ...
    md.write_text("# RViz/MoveIt Plan Preview Session (v1)\n\n"
                  ...
                  "## Optional fake-hardware smoke launch check\n\n"
                  "```bash\npython3 scripts/workcell_studio.py smoke-launch-preview \\\n+  --session <session_json> \\\n+  --output-dir <output_dir>/smoke_launch \\\n+  --dry-run \\\n+  --json\n+\n+# Optional/manual: actually launch fake-hardware visual stack for a short timeout.\npython3 scripts/workcell_studio.py smoke-launch-preview \\\n+  --session <session_json> \\\n+  --output-dir <output_dir>/smoke_launch \\\n+  --execute \\\n+  --timeout-s 20 \\\n+  --json\n+```\n", encoding='utf-8')
    sh.write_text("#!/usr/bin/env bash\n# Fake-hardware-only preview helper. DO NOT use for real hardware.\n# This script is generated and NOT executed automatically.\n# Optionally source your workspace first, e.g.: source install/setup.bash\n"+cmd+"\n\n# Optional fake-hardware smoke launch check\npython3 scripts/workcell_studio.py smoke-launch-preview \\\n+  --session \"$(dirname \"$0\")/rviz_moveit_plan_preview_session.json\" \\\n+  --output-dir \"$(dirname \"$0\")/smoke_launch\" \\\n+  --dry-run \\\n+  --json\n\n# Optional/manual execution path:\n# python3 scripts/workcell_studio.py smoke-launch-preview --session \"$(dirname \"$0\")/rviz_moveit_plan_preview_session.json\" --output-dir \"$(dirname \"$0\")/smoke_launch\" --execute --timeout-s 20 --json\n", encoding='utf-8')
    sh.chmod(0o755)
