...
def _build_preview_bundle(scene_pkg: Path, output_dir: Path, summary: dict[str, Any], export_payload: dict[str, Any], preview_payload: dict[str, Any]) -> Path:
    ...
    preview_json_src = Path(summary.get("preview_summary_json", ""))
    if preview_svg_src.is_file():
        shutil.copy2(preview_svg_src, bundle_dir / "environment_preview.svg")
    if preview_html_src.is_file():
        shutil.copy2(preview_html_src, bundle_dir / "environment_preview.html")
    if preview_json_src.is_file():
        shutil.copy2(preview_json_src, bundle_dir / "environment_preview_summary.json")

    missing = list(summary.get("missing_required_fields") or [])
    placeholders = []
    if not (export_payload.get("builder_task_intent") if isinstance(export_payload, dict) else {}):
        placeholders.append("builder_task_intent")
    ...
    (bundle_dir / "readiness_summary.html").write_text(
        "<html><body><h1>Readiness Summary</h1>"
        f"<p><b>Final readiness:</b> {final}</p>"
        "<p><b>Hardware mode default:</b> fake_hardware</p>"
        "<p><b>No-motion safety statement:</b> No launch, motion commands, or MoveIt services were executed by export.</p>"
        f"<p><b>Missing fields:</b> {', '.join(missing) if missing else 'none'}</p>"
        f"<p><b>Unsupported placeholders:</b> {', '.join(placeholders) if placeholders else 'none'}</p>"
        "</body></html>\n",
        encoding="utf-8",
    )
    ...
    launch_lines = [
        ...
        "",
        "## Fake hardware (default)",
        f"- `python3 scripts/create_workcell_project.py --cell-definition {summary.get('cell_definition_path')} --output-dir {output_dir} --force`",
    ]
    rviz_cmd = summary.get("rviz_plan_preview_suggested_commands_path")
    if rviz_cmd:
        launch_lines += ["", "## RViz / MoveIt preview", f"- `bash {rviz_cmd}`"]
    launch_lines += [
        "",
        "## Real hardware",
        "- Not generated by default. Explicit opt-in and guardrails required.",
    ]
...
def import_builder_scene(args: argparse.Namespace) -> int:
    ...
    preview_bundle_dir = _build_preview_bundle(scene_pkg, output_dir, summary, export_payload, preview_payload)
    summary["preview_bundle_dir"] = str(preview_bundle_dir)
