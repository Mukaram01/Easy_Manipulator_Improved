                                             ...
                                             const bool /*visual*/)
{
  ...
  std::string missing_elements = "Octomap: Missing element '" + std::string(OCTREE_ELEMENT_NAME);
#ifdef TESSERACT_PARSE_POINT_CLOUDS
  missing_elements += "' or '" + std::string(POINT_CLOUD_ELEMENT_NAME);
#endif
  missing_elements += "'; must define one!";

  std::throw_with_nested(std::runtime_error(missing_elements));
}
